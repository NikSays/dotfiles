...
const { findCentroid, findCornerForWindow } = Me.imports.src.utils.pointsArithmetic;
...
class MonitorGroup {
	...
	_getCloneCentroid(windowActorsClones) {
		const point = findCentroid(windowActorsClones.map(actorClone => {
			return {
	...
	_fillCloneDestPosition(windowActorsClones) {
		...
		windowActorsClones.map(actorClone => {
			...
			destCorner = destCorner !== null && destCorner !== void 0 ? destCorner : this._bottomMidCorner;
